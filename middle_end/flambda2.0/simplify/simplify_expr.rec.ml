...
= fun dacc let_symbol_expr k ->
    ...
    List.fold_left (fun dep_graph (bound_symbols, defining_expr) ->
        Code_id_or_symbol.Set.fold
          (fun (being_defined : Code_id_or_symbol.t) dep_graph ->
            ...
            | Code_id code_id ->
              begin match (defining_expr : Static.const.t) with
              | Code_and_set_of_closures { code; set_of_closures = _; } ->
                ...
                let free_names_newer_version_of =
                  ...
                  | Some newer_version_of ->
                    (* We treat this like a real code ID dependency, not a
                       "newer version of code ID" dependency. *)
                    Name_occurrences.add_code_id Name_occurrences.empty
                      newer_version_of NM.normal
                in
            ...
            | Symbol symbol ->
              ...
              | Code_and_set_of_closures { code_ids; closure_symbols; } ->
                ...
                match (defining_expr : Static.const.t) with
  ...
  let find_code code_id =
    let result =
      List.find_opt (fun (_bound_symbols, (static_const : Static_const.t)) ->
  ...
  let find_non_closure_or_closure_symbol sym =
    let result =
      List.find_opt
        (fun ((bound_symbols : Bound_symbols.t),
          ...
          | Singleton _ -> Non_closure static_const
    ...
    match result with
    | None ->
      Misc.fatal_errorf "Definition for symbol %a not found"
        Symbol.print symbol
    | Some result -> result
  in
            ...
            let code_ids, closure_symbols, code, set_of_closures =
              List.fold_left
                (fun (code_ids, closure_symbols, code, set_of_closures)
                  ...
                  | Symbol symbol ->
                    match find_code_or_closure_symbol symbol with
