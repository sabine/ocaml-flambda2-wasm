  ...
  try
    ...
    | `Textual, _ | `Original, Textual _ ->
      let unquote def =
        match def with
        | Textual m -> m
        | Encoded (_, bs) -> failwith "whatever" (* Decode.decode "" bs *)
        | Quoted (_, s) -> failwith "whatever2" (* unquote (Parse.string_to_module s) *)
    ...
    | `Binary, _ | `Original, Encoded _ ->
      let unquote def =
        ...
        | Encoded (_, bs) -> bs
        | Quoted (_, s) -> failwith "whatever3" (* unquote (Parse.string_to_module s) *)
