...
let lfield v i bi =
  ...
  in
  Lprim(Pfield (field_info, Reads_vary), [Lvar v], Location.none)
      ...
      | (_, path_lam, obj_init)::inh_init ->
          (inh_init,
           Llet (Strict, Pgenval, obj_init,
             mkappl(Lprim(class_field 1, [path_lam], Location.none),
               Lvar cla ::
                 if top then
                   [Lprim(class_field 3, [path_lam], Location.none)]
                 else []),
...
let transl_class ids cl_id pub_meths cl vflag =
  ...
  let inh_keys =
    List.map
      (fun (_, path_lam, _) ->
        Lprim(class_field 1, [path_lam], Location.none))
      inh_paths
  in
  let lclass lam =
